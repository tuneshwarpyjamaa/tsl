...
/**
 * Pre-warm cache with popular articles for instant loading
 */
export async function preWarmCache(req, res) {
  try {
    const { slugs = [] } = req.body;
    
    console.log(`[PERF] Pre-warming cache for ${slugs.length} articles...`);
    
    const promises = slugs.map(async (slug) => {
      try {
        const post = await Post.findBySlug(slug);
        if (post) {
          articleCache.set(`complete_post:${slug}`, {
            post: {
              ...post,
              categoryId: {
                name: post.category_name,
          ...
          console.log(`[PERF] Pre-warmed: ${slug}`);
          return { slug, status: 'success' };
        }
      } catch (error) {
        console.warn(`[PERF] Failed to pre-warm ${slug}:`, error.message);
    ...
    console.error('[PERF] Error in preWarmCache:', error);
    res.status(500).json({ 
...
export async function getCacheStats(req, res) {
  try {
    const stats = articleCache.getStats();
    res.json({
      success: true,
      data: stats
    });
  } catch (error) {
    console.error('[PERF] Error getting cache stats:', error);
    res.status(500).json({ 
